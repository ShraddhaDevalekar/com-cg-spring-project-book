...
public class BookController {
	...
	public ResponseEntity<Book> getBookByBookId(@PathVariable(name = "bookId") int bookId) {
		...
		Book b1 = bookService.getBookById(bookId);
		HttpStatus status = HttpStatus.OK;
		ResponseEntity<Book> response = new ResponseEntity<>(b1, status);
		return response;
	}
	...
	public ResponseEntity<List<Book>> getBookByBookName(@PathVariable(name = "bookName") String bookName) {
		LOG.info(bookName);
		HttpStatus status = HttpStatus.OK;
		ResponseEntity<List<Book>> response = new ResponseEntity<>(bookService.getBookByName(bookName),
				status);
		return response;
	}
